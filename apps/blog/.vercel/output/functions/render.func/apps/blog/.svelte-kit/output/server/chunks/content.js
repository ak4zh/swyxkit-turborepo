...
import { G as GH_USER_REPO, A as APPROVED_POSTERS_GH_USERNAME } from "./siteConfig.js";
import parse from "parse-link-header";
import slugify from "@sindresorhus/slugify";
import rehypeStringify from "rehype-stringify";
import rehypeSlug from "rehype-slug";
import rehypeAutoLink from "rehype-autolink-headings";
const remarkPlugins = void 0;
const rehypePlugins = [
  rehypeStringify,
  rehypeSlug,
  [
...
const allowedPosters = APPROVED_POSTERS_GH_USERNAME;
...
async function listContent() {
  ...
  do {
    const res = await fetch(
      (next == null ? void 0 : next.url) ?? `https://api.github.com/repos/${GH_USER_REPO}/issues?state=all&per_page=100`,
      {
        headers: authheader
      }
    );
...
async function getContent(slug) {
  ...
  if (blogpost) {
    const blogbody = blogpost.content.replace(
      /\n{% youtube (.*?) %}/g,
      (_, x) => {
    ...
    const content = (await compile(blogbody, {
      remarkPlugins,
      rehypePlugins
    })).code.replace(/>{@html `<code class="language-/g, '><code class="language-').replace(/<\/code>`}<\/pre>/g, "</code></pre>");
...
function parseIssue(issue) {
  ...
  return {
    ...
    description,
    category: ((_a = data.category) == null ? void 0 : _a.toLowerCase()) || "blog",
    tags,
